        ...
        let pages = parseInt(ile / 10);
          ...
          function createPagination(pages, page) {
            ...
            if (page > 1) {
              str += '<li class="page-item previous no"><a href="/transportations/'+pageCutLow+'" onclick="createPagination(pages, '+(page-1)+')"><</a></li>';
            }
            // Show all the pagination elements if there are less than 6 pages total
            if (pages < 6) {
            for (let p = 1; p <= pages; p++) {
              active = page == p ? "active" : "no";
              str += '<li class="'+active+'"><a href="/transportations/'+p+'" onclick="createPagination(pages, '+p+')">'+ p +'</a></li>';
            }
          }
